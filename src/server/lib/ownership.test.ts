...
describe("isUserOwned", () => {
  ...
  it("returns false for traversal attempt targeting plugin cache", () => {
    expect(isUserOwned("/tmp/../Users/me/.claude/plugins/cache/some-plugin/agent.md")).toBe(false);
  });

  it("returns true for path containing 'plugins/cache' substring outside claude home", () => {
    expect(isUserOwned("/tmp/plugins/cache/not-a-real-plugin/file.md")).toBe(true);
  });

  it("returns false for plugin cache path with .. in middle", () => {
    expect(isUserOwned("/Users/me/.claude/plugins/cache/foo/../bar/agent.md")).toBe(false);
  });
});
