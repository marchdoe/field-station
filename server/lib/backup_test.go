...
func TestBackupFile_CreatesBackupEntry(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(target, []byte(`{"key":"value"}`), 0o644))
...
func TestRestoreBackup_RestoresFile(t *testing.T) {
	...
	target := filepath.Join(claudeHome, "settings.json")
	require.NoError(t, os.WriteFile(target, []byte(`{"original":true}`), 0o644))

	backupDir := lib.BackupFile(target, lib.BackupOpUpdate, claudeHome)
	require.NotEmpty(t, backupDir)

	require.NoError(t, os.WriteFile(target, []byte(`{"modified":true}`), 0o644))
...
func TestPruneOldBackups_RemovesOldEntries(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(oldDir, "meta.json"), []byte(meta), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(oldDir, "file"), []byte("data"), 0o644))
